...
let
  # Upstream mismatch in the nixpkgs 1.5.0-beta.14 packaging (2026-08-07):
  # streamcontroller-streamdeck 0.1.7 dropped the legacy DeviceManager.USB_*
  ...
  # Drop the patch once nixpkgs ships a StreamController that matches
  # streamdeck 0.1.7.
  #
  ...
  streamcontroller = pkgs.streamcontroller.overrideAttrs (old: {
    buildInputs = old.buildInputs ++ [ pkgs.python3Packages.websockets ];
